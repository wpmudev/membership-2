...
class MS_Model_Gateway_Stripe extends MS_Model_Gateway {
	...
	public function process_purchase( $ms_relationship ) {
		do_action(
			'ms_model_gateway_stripe_process_purchase_before',
			$ms_relationship,
			$this
		);
		...
		return apply_filters(
			'ms_model_gateway_stripe_process_purchase',
			$invoice,
			$this
		);
	}
	...
	public function request_payment( $ms_relationship ) {
		do_action(
			'ms_model_gateway_stripe_request_payment_before',
			$ms_relationship,
			$this
		);
		...
		do_action(
			'ms_model_gateway_stripe_request_payment_after',
			$ms_relationship,
			$this
		);
	}
	...
	protected function get_stripe_customer( $member ) {
		...
		return apply_filters(
			'ms_model_gateway_stripe_get_stripe_customer',
			$customer,
			$member,
			$this
		);
	...
	protected function get_customer_id( $member ) {
		...
		return apply_filters(
			'ms_model_gateway_stripe_get_customer_id',
			$customer_id,
			$member,
			$this
		);
	...
	protected function save_customer_id( $member, $customer_id ) {
		...
		do_action(
			'ms_model_gateway_stripe_save_customer_id_after',
			$member,
			$this
		);
	}
	...
	public function add_card( $member, $token ) {
		...
		do_action(
			'ms_model_gateway_stripe_add_card_info_after',
			$member,
			$token,
			$this
		);
	...
	public function get_publishable_key() {
		...
		return apply_filters(
			'ms_model_gateway_stripe_get_publishable_key',
			$publishable_key
		);
	}
	...
	protected function get_secret_key() {
		...
		return apply_filters(
			'ms_model_gateway_stripe_get_secret_key',
			$secret_key
		);
	}
	...
	public function is_configured() {
		...
		return apply_filters(
			'ms_model_gateway_stripe_is_configured',
			$is_configured
		);
	}
}
