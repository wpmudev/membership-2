...
class MS_Gateway_Stripeplan extends MS_Gateway {
	 ...
	 * @return string Modified or raw URL
	 */
	public function ms_model_pages_get_ms_page_url_cb( $url, $page_type, $ssl, $site_id ) {
		 ...
		 *
		 * It's needed, if:
		 *   - You have no SSL certificate or don't want to use it.
		 *   - You use multiple gateways (e.g. Stripe + Paypal) and want to use
		 *     SSL on the M2 pages, because one gateway has problems with it.
		 *
		 * If the user has SSL certificate, this rule won't work
		 */
		if ( defined( 'M2_FORCE_NO_SSL' && M2_FORCE_NO_SSL ) ) {
			return $url;
		}

		if ( $this->active && $this->is_live_mode() ) {
			if ( MS_Model_Pages::MS_PAGE_MEMBERSHIPS == $page_type
				|| MS_Model_Pages::MS_PAGE_REGISTER == $page_type
			) {
	...
	/**
	 * Creates the external Stripe-ID of the specified Membership.
	 *
	 * @see self::get_the_id()
	 *
	 * @since  1.0.3.0
	 ...
	 * @param  bool                $legacy     If we need to return the legacy-ID.
	 * @return string The external Stripe-ID.
	 */
	static public function get_plan_id( $membership, $legacy = false ) {
		// Note: Name and revision are not included!
		$parts = array(
			(string) $membership->id,
			(string) $membership->price,
			(string) $membership->payment_type,
			(string) $membership->trial_period_unit,
			(string) $membership->trial_period_type,
			(string) $membership->pay_cycle_period_unit,
			(string) $membership->pay_cycle_period_type,
		);
		...
		return self::get_the_id( 'plan', $membership->id, $key );
	}

	/**
	 * Creates the external Stripe-ID of the specified coupon.
	 *
	 * @see self::get_the_id()
	 *
	 * @since  1.0.3.0
	 ...
	 * @return string The external Stripe-ID.
	 */
	static public function get_coupon_id( $coupon_id ) {
		return self::get_the_id( 'coupon', $coupon_id, $coupon_id );
	}
	 ...
	 * @since  1.0.0
	 * @internal
	 * @param  string $type The key-type: [plan|coupon].
	 * @param  int    $id   M2-internal ID.
	 * @param  string $key  The key-value, generated by other functions.
	 * @return string The external Stripe-ID.
	 */
	static protected function get_the_id( $type, $id, $key ) {
		static $Base = null;
		if ( null === $Base ) {
...
debug_log( '=> Stripe upd. plan start' );
		...
		$plan_data = array(
			'id' => self::get_plan_id( $membership ),
			'amount' => 0,
		);
...
debug_log( '=> Stripe upd. plan NOW', $plan_data );
	...
	public function update_stripe_data_coupon( $coupon ) {
		...
		$coupon_data = array(
			'id' => self::get_coupon_id( $coupon->id ),
			'duration' => $duration,
