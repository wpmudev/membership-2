...
class amazon extends M_Gateway {
	...
	function get_transactions($type, $startat, $num) {
		...
		switch($type) {
			...
			case 'past':
						$sql = $this->db->prepare( "SELECT SQL_CALC_FOUND_ROWS * FROM {$this->subscription_transaction} WHERE transaction_gateway = %s AND transaction_status IN ('Completed', 'Processed') ORDER BY transaction_stamp DESC LIMIT %d, %d", $this->gateway, $startat, $num );
						break;
			case 'pending':
						$sql = $this->db->prepare( "SELECT SQL_CALC_FOUND_ROWS * FROM {$this->subscription_transaction} WHERE transaction_gateway = %s AND transaction_status IN ('Pending') ORDER BY transaction_stamp DESC LIMIT %d, %d", $this->gateway, $startat, $num );
						break;
			case 'future':
						$sql = $this->db->prepare( "SELECT SQL_CALC_FOUND_ROWS * FROM {$this->subscription_transaction} WHERE transaction_gateway = %s AND transaction_status IN ('Future') ORDER BY transaction_stamp DESC LIMIT %d, %d", $this->gateway, $startat, $num );
						break;
